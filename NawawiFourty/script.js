(function () {
  ...
  // أزرار الاستيراد (موجودة الآن تحت خانة التعليقات)
  ...
  /* ثيم الليل/النهار مع تبديل الأيقونة */
  (function initTheme() {
    try {
    ...
    // ضبط افتراضي لو ما كان في قيمة محفوظة
    if (!document.body.dataset.theme) {
      document.body.dataset.theme = "dark";
    }

    function updateThemeIcon() {
      // لو الوضع نهاري → نعرض القمر (للانتقال لليل)
      // لو الوضع ليلي → نعرض الشمس (للانتقال للنهار)
    ...
    themeToggle.addEventListener("click", () => {
      ...
      try {
        localStorage.setItem("arbain_theme", newTheme);
      } catch (e) {}
      updateThemeIcon();
    });
  })();
  ...
  /* نسخ نص من عنصر معين إلى الحافظة */
  function fallbackCopy(text, label) {
    ...
    try {
      ...
      if (noteStatusEl) {
        noteStatusEl.textContent = `✅ تم نسخ ${label} إلى الحافظة.`;
      }
    } catch (e) {
      if (noteStatusEl) {
        noteStatusEl.textContent = `⚠️ تعذّر نسخ ${label}.`;
      }
    } finally {
  ...
  /* استيراد التعليقات من ملف نصي تم تصديره سابقًا */
  function importNotesFromText(content) {
    const lines = String(content).split(/\r?\n/);
    let currentIndexLocal = null;
    let buffer = [];

    const hadithRegex = /الحديث رقم\s+(\d+)/;

    function flushCurrent() {
      if (currentIndexLocal != null) {
        const noteText = buffer.join("\n").trim();
        if (noteText) {
          saveNote(currentIndexLocal, noteText);
        }
      }
      currentIndexLocal = null;
      buffer = [];
    }

    for (const line of lines) {
      const m = line.match(hadithRegex);
      if (m) {
        // بداية حديث جديد
        flushCurrent();
        currentIndexLocal = parseInt(m[1], 10);
        continue;
      }

      // نتجاوز سطر "تعليقاتي:"
      if (line.trim().startsWith("تعليقاتي:")) {
        continue;
      }
  ...
  if (importNotesBtn && importNotesInput) {
    ...
    importNotesInput.addEventListener("change", (e) => {
      ...
      reader.onload = () => {
        try {
          importNotesFromText(reader.result);
          noteStatusEl.textContent = "✅ تم استيراد التعليقات من الملف.";
          updateProgressBadge();
          renderHadithList();
          renderCurrentHadith();
        } catch (err) {
          console.error("خطأ أثناء استيراد التعليقات:", err);
          noteStatusEl.textContent = "⚠️ حدث خطأ أثناء استيراد التعليقات.";
        } finally {
  ...
  if (copyHadithBtn) {
    copyHadithBtn.addEventListener("click", () => {
      copyTextFromElement(hadithTextEl, "متن الحديث");
    });
  }

  if (copyExplanationBtn) {
    copyExplanationBtn.addEventListener("click", () => {
      copyTextFromElement(explanationTextEl, "شرح الحديث");
    });
  }
